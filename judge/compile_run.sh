...
while true; do
  ...
  if python3 /usr/local/bin/run_with_metrics.py \
    ...
    # Simple normalization: just remove trailing whitespace from each line
    sed 's/[[:space:]]*$//' "user_out_${i}.txt" > "user_out_${i}_norm.txt"
    sed 's/[[:space:]]*$//' "$OUT" > "expected_${i}_norm.txt"
    
    # Compare using diff with -Z (ignore trailing whitespace) and -B (ignore blank lines)
    if diff -ZB "user_out_${i}_norm.txt" "expected_${i}_norm.txt" > /dev/null 2>&1; then
      echo "TEST $i: OK"
      echo "OK" > "verdict_${i}.txt"
    else
      echo "TEST $i: WA (Wrong Answer)"
      echo "WA" > "verdict_${i}.txt"
      diff -ZB "user_out_${i}_norm.txt" "expected_${i}_norm.txt" > "diff_${i}.txt" 2>&1 || true
    fi
  else
    EXIT_CODE=$?
